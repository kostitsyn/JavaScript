class Board {
    ...
    /**
     * Если следующий шаг является шагом в стену, то змейка появится с противоположной стороны.
     * @param {Object} nextCellCoords - координаты ячейки, куда змейка собирается сделать шаг.
     * @param {number} nextCellCoords.x
     * @param {number} nextCellCoords.y
     */
    isNextStepAfterWall(nextCellCoords) {
        if (nextCellCoords.x == 0 && this.snake.direction == 'left') {
            nextCellCoords.x = 21;
        }
        if (nextCellCoords.x == 22 && this.snake.direction == 'right') {
            nextCellCoords.x = 1;
        }
        if (nextCellCoords.y == 0 && this.snake.direction == 'up') {
            nextCellCoords.y = 21;
        }
        if (nextCellCoords.y == 22 && this.snake.direction == 'down') {
            nextCellCoords.y = 1;
        }
    }
...
class Game {
    ...
    doTick() {
        ...
        this.food.setFood();
        this.board.isNextStepAfterWall(this.snake.body[0]);
     ...
     */
    isGameLost() {
        //if (this.board.isNextStepToWall(this.snake.body[0]) || this.snake.isEatHimself(this.snake.body[0])) {
        if (this.snake.isEatHimself(this.snake.body[0])) {
            clearInterval(this.tickIdentifier);
    ...
    /**
     * Метод выводит счёт игры.
     * @param {string} text 
     */
    showScore() {
        this.score++;
        this.scoreBoardEl.innerText = `Текущий счёт: ${this.score - 1}`;
    }
}   
...
class Snake {
     ...
     * Если ест то наступает проигрыш.
     */
    isEatHimself(newHeadCoords) {
        for (let i in this.body.slice(1)) {
            if (JSON.stringify(this.body.slice(1)[i]) == JSON.stringify(newHeadCoords)) {
                return true;
            } 
        }
        return false;
